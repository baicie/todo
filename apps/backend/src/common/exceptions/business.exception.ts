...
export class UserNotFoundException extends BusinessException {
  constructor(message = '用户不存在', translationKey = 'user.notFound') {
    super(message, ErrorCode.USER_NOT_FOUND, HttpStatus.NOT_FOUND, translationKey);
  }
}

export class ProductNotFoundException extends BusinessException {
  constructor(message = '商品不存在', translationKey = 'product.notFound') {
    super(message, ErrorCode.PRODUCT_NOT_FOUND, HttpStatus.NOT_FOUND, translationKey);
  }
}

export class OrderNotFoundException extends BusinessException {
  constructor(message = '订单不存在', translationKey = 'order.notFound') {
    super(message, ErrorCode.ORDER_NOT_FOUND, HttpStatus.NOT_FOUND, translationKey);
  }
}

export class InsufficientStockException extends BusinessException {
  constructor(message = '库存不足', translationKey = 'product.insufficientStock') {
    super(message, ErrorCode.INSUFFICIENT_STOCK, HttpStatus.BAD_REQUEST, translationKey);
  }
}

export class InvalidCredentialsException extends BusinessException {
  constructor(message = '用户名或密码错误', translationKey = 'auth.login.invalidCredentials') {
    super(message, ErrorCode.INVALID_CREDENTIALS, HttpStatus.UNAUTHORIZED, translationKey);
  }
}
